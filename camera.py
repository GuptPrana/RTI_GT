...
class Camera:
  ...
  def transformation_matrix(self):
    transform = np.hstack((self.rotation_matrix(), np.array(self.translation)))
  ...
  def inverse_transformation_matrix(self):
    ...
    return np.vstack((np.hstack((rotation_t, translation_t)), np.array([0, 0, 0, 1]))) 
  
  def transform_pointclouds(self, pointcloud, transformation_matrix):
