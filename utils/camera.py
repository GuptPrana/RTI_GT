...
class Camera:
    ...
    def transformation_matrix(self):
        transform = np.column_stack(
            (self.rotation_matrix(), np.array(self.translation))
        )
    ...
    def inverse_transformation_matrix(self):
        ...
        return np.vstack(
            (np.column_stack((rotation_t, translation_t)), np.array([0, 0, 0, 1]))
        )
    ...
    def transform_pointcloud(self, pointcloud, transformation_matrix):
        ...
        inverse_transformed = (
            self.inverse_transformation_matrix() @ homogeneous_points.transpose()
        ).transpose()
        transformed_points = (
            transformation_matrix @ inverse_transformed.transpose()
        ).transpose()[:, :3]
